"""Unit tests for the builtin capability provider's ``run_command`` payload.

The provider owns the single ``run_command`` implementation: it resolves the call
into a :class:`CommandSpec` and hands it to a per-run :class:`CommandSession`, so
*where* the command runs is decided entirely by the injected backend. These tests
exercise the spec assembly, the result mapping, and the session lifecycle with a
fake backend, plus one end-to-end run against the real local backend.
"""
...
def _command_run(command, *, timeout_seconds: int = 600, **args) -> TaskRun:
    ...
    spec = TaskSpec(
        task_id="t-cmd",
        title="Run a command",
        dispatch=CapabilityDispatch(capability_id="builtin:run_command"),
        args=payload,
    )
...
def _file_convert_run(**args) -> TaskRun:
    spec = TaskSpec(
        ...
        dispatch=CapabilityDispatch(capability_id="builtin:file_convert"),
        args=args,
    )
    return _run(spec)
...
def test_builtin_provider_requires_dependencies(tmp_path: Path) -> None:
    artifact_store = FileArtifactStore(tmp_path / "artifacts")
    backend = _FakeBackend(CommandResult(return_code=0))
    with pytest.raises(ValueError, match="artifact_store is required"):
        BuiltinCapabilityProvider(artifact_store=None, command_backend=backend)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="command_backend is required"):
        BuiltinCapabilityProvider(artifact_store=artifact_store, command_backend=None)  # type: ignore[arg-type]


@pytest.mark.asyncio
async def test_unknown_capability_is_user_input_failure(tmp_path: Path) -> None:
    executor = _tool_executor(tmp_path, _FakeBackend(CommandResult(return_code=0)))
    run = _run(TaskSpec(task_id="t", title="T", dispatch=CapabilityDispatch(capability_id="builtin:nope")))

    result = await executor.run(run, _FakeStore())

    assert result.status == TaskStatus.FAILED
    assert result.error_class == ErrorClass.USER_INPUT
    assert "builtin:nope" in result.summary
...
async def test_file_convert_converts_xlsx_to_csv(tmp_path: Path) -> None:
    ...
    executor = _tool_executor(tmp_path, _FakeBackend(CommandResult(return_code=0)))
    result = await executor.execute(_file_convert_run(input_paths=["attachments/cases.xlsx"], sheet="Cases"))
...
@pytest.mark.asyncio
async def test_run_command_mounts_only_trusted_source_object(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    source_ref = "attachments/commands.csv"
    source_path = workspace / source_ref
    source_path.parent.mkdir(parents=True)
    source_path.write_bytes(b"Command\necho authorized\n")
    ...
    backend = _FakeBackend(CommandResult(return_code=0))
    executor = _tool_executor(tmp_path, backend)
    run = _command_run(object_ref)
    run.spec.metadata["tabular"] = {"source_ref": source_ref, "source_object_ref": object_ref}

    await executor.run(run, _FakeStore())

    (spec,) = backend.ran_specs
    assert spec.argv == ["sh", "-lc", str(object_path)]
    assert [mount.name for mount in spec.mounts] == ["result", "workspace", "source-input-0"]
    source_mount = spec.mounts[-1]
    assert source_mount.host_path == source_mount.mount_path == str(object_path.parent)
    assert source_mount.read_only


@pytest.mark.asyncio
async def test_run_command_does_not_mount_source_object_unused_by_args(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    source_ref = "attachments/commands.csv"
    source_path = workspace / source_ref
    source_path.parent.mkdir(parents=True)
    source_path.write_bytes(b"Command\necho authorized\n")
    manifest = WorkspaceSourceService().index_path(workspace, source_ref, source_path)
    object_ref = WorkspaceSourceRepository(workspace).object_ref(manifest)
    backend = _FakeBackend(CommandResult(return_code=0))
    executor = _tool_executor(tmp_path, backend)
    run = _command_run("echo selected row")
    run.spec.metadata["tabular"] = {"source_ref": source_ref, "source_object_ref": object_ref}

    await executor.run(run, _FakeStore())

    (spec,) = backend.ran_specs
    assert [mount.name for mount in spec.mounts] == ["result", "workspace"]


@pytest.mark.asyncio
async def test_run_command_does_not_mount_untrusted_source_uri(tmp_path: Path) -> None:
    object_ref = "sico-source://objects/deadbeef/source.sh"
    backend = _FakeBackend(CommandResult(return_code=0))
    executor = _tool_executor(tmp_path, backend)

    await executor.run(_command_run(object_ref), _FakeStore())

    (spec,) = backend.ran_specs
    assert spec.argv == ["sh", "-lc", object_ref]
    assert [mount.name for mount in spec.mounts] == ["result", "workspace"]


@pytest.mark.asyncio
async def test_run_command_rejects_missing_trusted_source_object(tmp_path: Path) -> None:
    object_ref = "sico-source://objects/deadbeef/source.sh"
    backend = _FakeBackend(CommandResult(return_code=0))
    executor = _tool_executor(tmp_path, backend)
    run = _command_run(object_ref)
    run.spec.metadata["tabular"] = {
        "source_ref": "attachments/missing.sh",
    ...
    assert "unavailable or corrupt source object" in result.error_message
    assert backend.ran_specs == []


@pytest.mark.asyncio
async def test_run_command_keeps_prepared_object_after_logical_source_replacement(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    source_ref = "attachments/commands.csv"
    source_path = workspace / source_ref
    source_path.parent.mkdir(parents=True)
    ...
    old_manifest = service.index_path(workspace, source_ref, source_path)
    repository = WorkspaceSourceRepository(workspace)
    old_object_ref = repository.object_ref(old_manifest)
    source_path.write_bytes(b"Command\nnew\n")
    service.index_path(workspace, source_ref, source_path)
    backend = _FakeBackend(CommandResult(return_code=0))
    executor = _tool_executor(tmp_path, backend)
    run = _command_run(old_object_ref)
    run.spec.metadata["tabular"] = {"source_ref": source_ref, "source_object_ref": old_object_ref}

    result = await executor.run(run, _FakeStore())

    assert result.status == TaskStatus.COMPLETED
    (spec,) = backend.ran_specs
    assert spec.argv == ["sh", "-lc", str(repository.resolve_object_ref(old_object_ref))]
