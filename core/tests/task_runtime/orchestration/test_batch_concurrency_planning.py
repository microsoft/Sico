...
def test_caller_concurrency_can_only_narrow_deployment_limit() -> None:
    assert _batch_concurrency_limit(20, 4) == 4
    assert _batch_concurrency_limit(20, 40) == 20
    assert _batch_concurrency_limit(20, None) == 20
...
def test_resource_gate_buckets_each_sandbox_type_independently() -> None:
    ...
    plan = BatchExecutionPlan(
        ...
        sandbox_plans=(SandboxTypePlan(sandbox_type="emulator", task_count=5, concurrency=3, available_count=3),),
    )
    limits = _execution_resource_limits(plan)
...
def test_ordered_sandbox_types_follows_canonical_priority() -> None:
    ...
    assert _ordered_sandbox_types({"linux": 3, "android": 5}) == ["android", "linux"]
...
def test_choose_sandbox_option_prefers_available_capacity() -> None:
    assert (
        _choose_sandbox_option(
            ("windows", "macos"),
            counts={"windows": 2},
            available_by_type={"windows": 2, "macos": 1},
        )
        == "macos"
    )
