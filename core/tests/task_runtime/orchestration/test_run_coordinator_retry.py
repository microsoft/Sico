...
from app.biz.task_runtime.events.bus import RunStateTransition, RuntimeEventBus, get_default_bus, set_default_bus
...
from app.biz.task_runtime.storage.run_store import StaleWorkerError
...
def _failed_run(attempt: int = 1) -> TaskRun:
    spec = TaskSpec(task_id="A", title="A", dispatch=CapabilityDispatch(capability_id="builtin:echo"))
    return TaskRun(
        run_id="run-A",
        batch_id="batch-1",
...
async def test_prepare_retry_reopens_same_row_and_increments_attempt(captured_transitions, monkeypatch) -> None:
    import app.biz.task_runtime.orchestration.run_coordinator as rc_module
    ...
    assert [(e.from_status, e.to_status) for e in captured_transitions] == [(TaskStatus.FAILED, TaskStatus.QUEUED)]
