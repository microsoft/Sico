...
from app.biz.task_runtime.orchestration.scheduler import _group_runs_by_stage, _stage_gate_blocks


def _run(task_id: str, stage: int, batch_item_index: int = 0) -> TaskRun:
    spec = TaskSpec(task_id=task_id, title=task_id, dispatch=CapabilityDispatch(capability_id="builtin:echo"), stage=stage)
    return TaskRun(
...
def test_stage_defaults_to_zero():
    spec = TaskSpec(task_id="t", title="T", dispatch=CapabilityDispatch(capability_id="builtin:echo"))
    assert spec.stage == 0


def test_negative_stage_is_clamped():
    spec = TaskSpec(task_id="t", title="T", dispatch=CapabilityDispatch(capability_id="builtin:echo"), stage=-3)
    assert spec.stage == 0
