...
from app.biz.task_runtime.orchestration.scheduler import BatchScheduler


def _run(task_id: str, index: int) -> TaskRun:
    spec = TaskSpec(task_id=task_id, title=task_id, dispatch=CapabilityDispatch(capability_id="builtin:echo"))
    return TaskRun(
