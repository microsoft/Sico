...
from app.biz.task_runtime.events.bus import BatchStateTransition, RuntimeEventBus
from app.biz.task_runtime.domain.models import BatchStatus, CapabilityDispatch, TaskStatus
...
def _make_run(
    *,
    ...
    scheduled_batch_id: str | None = None,
    conv: int = 100,
    turn: int = 200,
    project: int = 7,
    ...
    dispatch = CapabilityDispatch(capability_id=f"skill:{skill_name}.run" if skill_name is not None else "builtin:echo")
    spec = SimpleNamespace(dispatch=dispatch, task_id="task-X", skill_name=skill_name)
    return SimpleNamespace(
        run_id=run_id,
        batch_id=batch_id,
        scheduled_batch_id=scheduled_batch_id or batch_id,
...
def test_non_skill_run_is_noop(scheduled: list[str], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # skill_name=None builds a builtin-capability run -> _skill_name returns "" -> no-op.
...
def test_per_batch_drains_a_nested_call_with_its_scheduling_batch(
    scheduled: list[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # A nested capability call sits in a synthetic batch of its own that never
    # reaches a terminal transition. Buffering it under that batch would strand
    # it forever and leak the bookkeeping entry, so it follows the batch that
    # scheduled its parent.
    monkeypatch.setenv("EPE_TRIGGER_MODE", "per_batch")
    bus = RuntimeEventBus()
    experience_learning.register(bus)

    experience_learning.on_run_terminal(
        _make_run(run_id="run-a-step-1", batch_id="run-a-calls", scheduled_batch_id="batch-9"),
        _make_result(),
    )
    assert "run-a-calls" not in experience_learning._BATCH_META

    bus.publish(BatchStateTransition(batch_id="batch-9", from_status=BatchStatus.RUNNING, to_status=BatchStatus.PARTIAL))

    assert scheduled == ["run-a-step-1"]
    assert "run-a-calls" not in experience_learning._BATCH_META
    assert "batch-9" not in experience_learning._BATCH_META
