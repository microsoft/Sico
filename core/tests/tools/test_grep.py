...
def test_resolve_search_targets_rejects_internal_source_storage(tmp_path) -> None:
    internal = tmp_path / ".source-repository"
    internal.mkdir()

    with pytest.raises(ValueError, match="workspace-relative"):
        _resolve_search_targets(tmp_path, [".source-repository"])
