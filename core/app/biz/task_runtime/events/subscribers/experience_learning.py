...
The per-run trigger is **inline**: the capability executor calls
...
from ..bus import BatchStateTransition, RuntimeEvent
from ...domain.models import TaskRun
from ...domain.state_machine import is_terminal_batch_status
...
def on_run_terminal(run: TaskRun, result: "TaskResult") -> None:
    ...
    Called by the capability executor right after a run produces its result, so
    ...
    if not _skill_name(run):
        return
    # Keyed on the *scheduling* batch, not the run's own: a nested capability
    # call sits in a batch of its own that never emits a terminal transition, so
    # keying on it would buffer the run forever and leak the meta entry.
    scheduled_batch_id = run.scheduled_batch_id
    ...
    if mode == "disabled":
        return
    if mode == "per_batch":
        _PENDING_BATCH_RUNS.setdefault(scheduled_batch_id, []).append((run, result))
        return
    _LOGGER.info(
...
def _schedule_dispatch(run: TaskRun, result: "TaskResult") -> None:
    ...
    bucket = _BATCH_TASKS.setdefault(run.scheduled_batch_id, set())
    bucket.add(task)
    task.add_done_callback(bucket.discard)


def _run_dir(run: TaskRun) -> Path:
    """Replicate the capability executor's per-run working directory layout."""
...
async def _dispatch(run: TaskRun, result: "TaskResult") -> None:
    ...
    Works purely with the in-memory ``(run, result)`` (``spec.skill_name``
