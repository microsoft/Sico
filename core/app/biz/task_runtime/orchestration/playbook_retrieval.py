...
consumers (``presentation/rendering/tool_payload.py`` and the skill capability
provider) read those args back out — they are the matching consumer side.
"""
...
def attach_playbook_hints(ctx: TurnContext, task: TaskSpec) -> TaskSpec:
    ...
    The skill provider keeps a non-empty ``args["instructions"]`` in preference
