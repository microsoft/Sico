...
class InMemorySandboxLeaseManager:
    ...
    async def acquire(self, token: ReservationToken) -> SandboxLeaseRef:
        ...
        # backend resolves the real machine). An OS with no fixed-OS type yet has
...
class ReverseGrpcSandboxLeaseManager:
    ...
    async def acquire(self, token: ReservationToken) -> SandboxLeaseRef:
        ...
        now_ms = _now_ms()
        # New backends return explicit lease metadata. During rollout, older
        # compatible backends omit it, so retain the sandbox-id prefix fallback.
        lease_type = (getattr(result, "provider_type", "") or "").strip() or lease_type_from_sandbox_id(result.applied_sandbox_id)
        ...
        if lease_type is None:
            ...
            reason = (
                f"reserved a {token.type} sandbox but the backend applied "
                f"{result.applied_sandbox_id!r}, which is {lease_type} ({supplied_os})"
            )
        else:
            reason = ""
        if reason:
            # Substituting a plausible type here - the previous behaviour - is
            # worse than failing. It is indistinguishable from a real one
            # downstream, and because it is drawn from ``eligible_types_for_os``
            # it silently satisfies the very capability-side check that exists to
            # catch this. The sandbox is already applied to this instance, so it
            # has to be handed back or the machine leaks; SANDBOX_UNHEALTHY is
            # retryable, so the run gets a different one instead of executing on
            # a sandbox whose OS could not be confirmed.
            try:
                await asyncio.to_thread(self.service.release_sandbox, self.agent_instance_id, result.applied_sandbox_id)
            except Exception:
                # Reclaiming the machine is best effort - the rejection stands
                # either way. Swallowing the failure, though, makes a leak read
                # exactly like a clean hand-back in the log, so the one outcome
                # an operator has to act on is the one nothing reports.
                _LOGGER.warning(
                    "failed to release rejected sandbox run_id=%s sandbox_id=%s",
                    token.run_id,
                    result.applied_sandbox_id,
                    exc_info=True,
                )
            _LOGGER.warning("sandbox lease rejected run_id=%s reason=%s", token.run_id, reason)
            raise SandboxUnhealthyError(reason)
        return SandboxLeaseRef(
            sandbox_id=result.applied_sandbox_id,
            type=lease_type,
            os=supplied_os or token.type,
