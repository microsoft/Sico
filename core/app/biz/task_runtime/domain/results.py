...
- **Constructors** (``blocked_result`` / ``failed_result`` / ``timed_out_result`` / ``cancelled_result``,
...
from ..storage.run_store import StaleWorkerError
from .time import now_ms as _now_ms
...
def timed_out_result(run: TaskRun, message: str) -> TaskResult:
    now_ms = _now_ms()
    started_at = run.started_at or run.queued_at or now_ms
    return TaskResult(
        run_id=run.run_id,
...
async def ensure_timed_out_state(store: RunStore, run: TaskRun, result: TaskResult) -> TaskResult:
    """Persist a timeout after cancelling the executor that owned the claim."""
    try:
        detail = await store.get_task_detail(run.run_id, "summary")
    except Exception:
        detail = None
    settled_result = _settled_result_from_detail(detail, result) if detail is not None else None
    if settled_result is not None:
        return settled_result
    current = detail.run if detail is not None else run
    await persist_stranded_result(store, current, result)
    try:
        detail = await store.get_task_detail(run.run_id, "summary")
    except Exception:
        return result
    return _settled_result_from_detail(detail, result) or result
