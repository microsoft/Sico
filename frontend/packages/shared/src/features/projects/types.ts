...
export type AssetCreator =
  ...
  | {
      ...
      iconUrl?: string;
      agentName?: string;
      // The human operator who ran the DW — the `.own` delete subject for
      // Deliverable/Experience (PRD: "DW Operator" may delete). Optional: older
      // rows predate the backend field, so a missing value fails `.own` closed.
