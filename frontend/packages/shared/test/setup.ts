...
// Activate the global i18n singleton so `t`/`<Trans>` macros (compiled to
// `i18n._(...)`) don't throw "called a translation function without setting a
// locale". Empty catalog => macros fall back to their source `message`, so
// tests keep asserting against the English strings.
