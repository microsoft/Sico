...
describe("projectAgentInstanceSchema", () => {
  it("renames the backend agentIconUrl to iconUrl", () => {
  ...
  it("coerces a null agentIconUrl to an empty string (Go marshals an unset avatar as null)", () => {
    const parsed = projectAgentInstanceSchema.parse({
      id: 1,
      agentIconUrl: null,
    });
...
describe("projectSchema", () => {
  ...
  it("accepts a full project payload and strips unknown fields", () => {
    ...
    // The agent digest's `agentIconUrl` is renamed to the canonical `iconUrl`.
