...
describe("fetchAgents", () => {
  ...
  it("adds employerUsername when scoped to a user's employed DWs", async () => {
    const client = makeClient({
      code: 0,
      msg: "ok",
      data: { instances: [], total: 0, hasNext: false },
    });
    await fetchAgents(client, { page: 1, employerUsername: "me@x.com" });
    expect(client.get).toHaveBeenCalledWith("/agent/single_agent_instances", {
      params: {
        page: 1,
        pageSize: 30,
  ...
  it("preserves parsed conversationStatus in result items", async () => {
    const client = makeClient({
      ...
      msg: "ok",
      data: {
        instances: [{ id: 1, name: "A", conversationStatus: 1 }],
        total: 1,
        hasNext: false,
      },
    });
    const result = await fetchAgents(client, { page: 1 });
    expect(result.items[0]?.conversationStatus).toBe(1);
  });
