...
func registerSandboxRoutes(r *gin.RouterGroup) {
	...
	// Emulator app management APIs (Dashboard / frontend)
	sandboxApi.POST("/emulator/apps/list", handler.EmulatorListApps)
	sandboxApi.POST("/emulator/apps/install", handler.EmulatorInstallApp)
	sandboxApi.GET("/emulator/apps/tasks/:taskId", handler.GetEmulatorAppTask)
	sandboxApi.POST("/emulator/apps/uninstall", handler.EmulatorUninstallApp)
