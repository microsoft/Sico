...
func (s *Service) PublishSingleAgent(
	...
	if _, ok := single_agent.SingleAgentPublishStatus_name[int32(targetStatus)]; !ok {
		return nil, apperr.New(errcode.CommonInvalidParam, "invalid publish status")
	}
	...
	if err := s.withRepositories(ctx, func(
		...
		return agentRepo.UpdatePublishStatus(ctx, agent.AgentId, targetStatus)
	}); err != nil {
		return nil, err
	}
