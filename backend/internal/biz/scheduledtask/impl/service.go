...
type Components struct {
	...
	AgentInstanceRepo   agentrepository.SingleAgentInstanceRepository
...
func (s *Service) poll(ctx context.Context) error {
	...
	for _, task := range tasks {
		...
		var agentInstanceDigest *commondto.AgentInstanceDigest
		agentInstance, agentInstanceErr := s.AgentInstanceRepo.Get(ctx, task.AgentInstanceID)
		if agentInstanceErr != nil {
			logger.CtxError(
				ctx, "scheduled task agent instance lookup failed id=%d err=%v", task.ID, agentInstanceErr,
			)
		} else if agentInstance != nil {
			agentInstanceDigest = &commondto.AgentInstanceDigest{
				Id:               agentInstance.Id,
				AgentName:        agentInstance.Name,
				AgentIconUrl:     agentInstance.IconUri,
				OperatorUsername: agentInstance.OperatorUsername,
			}
		}
		run, claimed, claimErr := s.Repository.Claim(
			...
			&entity.RunExtraInfo{
				...
				AgentInstance:       agentInstanceDigest,
	...
	notificationID, err := s.NotificationService.Create(ctx, &notificationentity.Notification{
		...
		ExtraInfo: &notificationdto.NotificationExtraInfo{
			ScheduledTaskFinished: &notificationdto.NotificationExtraInfoScheduledTaskFinished{
				...
				AgentInstance:      run.ExtraInfo.AgentInstance,
