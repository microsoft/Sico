...
func (c *Service) GetConversation(ctx context.Context,
	...
	resp.Data = &model.GetConversationData{
		Conversation: &model.ConversationData{
			...
			ExtraInfo:         conversationData.ExtraInfo,
		},
	}
	return resp, nil
}
...
func (c *Service) ListConversation(
	...
	conversationIDs := make([]int64, 0, len(conversationDOList))
	...
	for _, conv := range conversationDOList {
		...
		conversationData = append(conversationData, &model.ConversationListItem{
			...
			Title:           conv.Title,
			ExtraInfo:       conv.ExtraInfo,
		})
	}
	if req.GetFetchConversationStatus() {
		statuses := c.GetConversationRunStatuses(ctx, conversationIDs)
		for _, conversation := range conversationData {
