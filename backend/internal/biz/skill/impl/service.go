...
func (s *Service) CreateSkill(ctx context.Context, req *skill.CreateSkillRequest) (*skill.CreateSkillResponse, error) {
	...
	defer func() {
		...
		defer cancel()
		if deleteVerErr := s.SkillRepo.DeleteVersions(cleanupCtx, id); deleteVerErr != nil {
			logger.CtxWarn(ctx, "skill: failed to delete skill versions after "+
				"creation failure: id=%d err=%v", id, deleteVerErr)
		}
		if deleteErr := s.SkillRepo.Delete(cleanupCtx, id); deleteErr != nil {
			logger.CtxError(ctx, "skill: failed to delete skill after "+
				"creation failure: id=%d err=%v", id, deleteErr)
		}
	}()
