...
func TestEnsureTokenRejectsEmptyServerToken(t *testing.T) {
	...
	// they never claimed).
	row := &runRow{RunID: "r1", FencingToken: ""}
...
func TestEnsureClaimableAllowsQueued(t *testing.T) {
	row := &runRow{RunID: "r1", Status: statusQueued}
...
func TestEnsureClaimableRejectsTerminalStatuses(t *testing.T) {
	for _, status := range []string{statusRunning, "completed", "failed", "cancelled", "timed_out", "blocked"} {
		row := &runRow{RunID: "r1", Status: status}
...
func TestEnsureReopenableAllowsRetryableTerminalAtExpectedAttempt(t *testing.T) {
	for _, status := range []string{statusFailed, statusTimedOut, statusBlocked} {
		row := &runRow{RunID: "r1", Status: status, Attempt: 1}
...
func TestEnsureReopenableRejectsAttemptMismatch(t *testing.T) {
	...
	row := &runRow{RunID: "r1", Status: statusFailed, Attempt: 2}
	err := ensureReopenable(row, 1)
	if err == nil {
...
func TestEnsureReopenPayloadRejectsStaleRunState(t *testing.T) {
	...
	started := int64(5)
	mutators := map[string]func(*runRow){
		"worker_id":     func(p *runRow) { p.WorkerID = "worker-9" },
		"fencing_token": func(p *runRow) { p.FencingToken = "tok" },
		"started_at":    func(p *runRow) { p.StartedAt = &started },
		"ended_at":      func(p *runRow) { p.EndedAt = &started },
	}
...
func TestEnsureReopenPayloadRejectsIdentityChange(t *testing.T) {
	...
	mutators := map[string]func(*runRow){
		"batch_id":               func(p *runRow) { p.BatchID = "other" },
		"idempotency_key":        func(p *runRow) { p.IdempotencyKey = "other" },
		"batch_item_index":       func(p *runRow) { p.BatchItemIndex = 9 },
		"task_id":                func(p *runRow) { p.TaskID = "other" },
		"parent_conversation_id": func(p *runRow) { p.ParentConversationID = 99 },
		"parent_turn_id":         func(p *runRow) { p.ParentTurnID = 99 },
	}
	for field, mutate := range mutators {
		p := reopenPayload()
		mutate(p)
...
func TestDuplicateRunCreateMatchesExistingRequiresSameIdempotencyKey(t *testing.T) {
	existing := runRow{RunID: "run-1", BatchID: "batch-1", IdempotencyKey: "key-1"}

	if !duplicateRunCreateMatchesExisting(existing,
		runRow{RunID: "run-1", BatchID: "batch-1", IdempotencyKey: " key-1 "},
	) {
		t.Fatal("expected exact run/idempotency retry to be accepted")
	}
	if duplicateRunCreateMatchesExisting(existing,
		runRow{RunID: "run-1", BatchID: "batch-1", IdempotencyKey: "key-2"},
	) {
		t.Fatal("same run_id with a different idempotency key must be treated as a collision")
	}
	if duplicateRunCreateMatchesExisting(existing,
		runRow{RunID: "run-2", BatchID: "batch-1", IdempotencyKey: "key-1"},
	) {
		t.Fatal("same idempotency key with a different run_id must be treated as a collision")
	}
	if duplicateRunCreateMatchesExisting(existing,
		runRow{RunID: "run-1", BatchID: "batch-2", IdempotencyKey: "key-1"},
	) {
	...
	if duplicateRunCreateMatchesExisting(
		runRow{RunID: "run-1", BatchID: "batch-1"},
		runRow{RunID: "run-1", BatchID: "batch-1"},
	) {
